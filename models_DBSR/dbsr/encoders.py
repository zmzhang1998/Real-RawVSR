...
class ResEncoderWarpAlignnet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_rgb = torch.stack((x[:, :, 0], x[:, :, 1:3].mean(dim=2), x[:, :, 3]), dim=2)
        x_ref = x_rgb[:, 2:3, ...].repeat(1, x_rgb.shape[1] - 1, 1, 1, 1).contiguous()
        x_oth = torch.cat((x_rgb[:, 0:2, ...].contiguous(), x_rgb[:, 3:, ...].contiguous()), dim=1)
        ...
        ref_feat = feat[:, 2:3, ...].contiguous()
        oth_feat = torch.cat((feat[:, 0:2, ...].contiguous(), feat[:, 3:, ...].contiguous()), dim=1)
