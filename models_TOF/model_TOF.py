...
from models_TOF.common import flow_warp
from models_TOF.registry import BACKBONES
from models_TOF.utils import get_root_logger
...
class TOFlow(nn.Module):
    ...
    def __init__(self, adapt_official_weights=False, nframes=5, scale=4):
        super().__init__()
        self.adapt_official_weights = adapt_official_weights
        self.ref_idx = 0 if adapt_official_weights else nframes // 2
        self.frames = nframes
        self.scale = scale
        ...
        self.conv1 = nn.Conv2d(3 * 5, 64, 9, 1, 4)
    ...
    def forward(self, lrs):
        """
        Args:
            lrs: Input lr frames: (b, 5, 3, h, w).

        Returns:
            Tensor: SR frame: (b, 3, h, w).
        """
        # In the official implementation, the 0-th frame is the reference frame
        lrs_up = torch.zeros(*lrs.shape[:3], lrs.shape[3]*self.scale, lrs.shape[4]*self.scale).cuda()
        for i in range(self.frames):
            lrs_up[:, i] = F.interpolate(lrs[:, i], scale_factor=self.scale, mode='bicubic', align_corners=False)

        if self.adapt_official_weights:
            lrs_up = lrs_up[:, [2, 0, 1, 3, 4], :, :, :]

        num_batches, num_lrs, _, h, w = lrs_up.size()

        lrs_up = self.normalize(lrs_up.view(-1, 3, h, w))
        lrs_up = lrs_up.view(num_batches, num_lrs, 3, h, w)

        lr_ref = lrs_up[:, self.ref_idx, :, :, :]
        lr_aligned = []
        for i in range(self.frames):  # 7 frames
            if i == self.ref_idx:
