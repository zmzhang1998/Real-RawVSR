...
parser.add_argument('--model', type=str, default='model_EDVR', help='base model')
...
from models_EDVR_M.model_EDVR import EDVR
import utils
...
# load model
net = EDVR(nframes=opt.N_frames, scale=opt.scale)
net = net.cuda()
...
for test_data in tqdm(test_dataloader):
    ...
    with torch.no_grad():
        ...
        # Save image
        if opt.save_image:
            utils.save_RGB(HR_RGB_cor, str(opt.scale), test_data['RGB_gt_name'], 'EDVR')
