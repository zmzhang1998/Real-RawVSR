...
parser.add_argument('--model', type=str, default='model_DBSR', help='base model')
...
from models_DBSR.dbsr import dbsrnet
import utils
...
# load model
net = dbsrnet.dbsrnet_cvpr2021(upsample_factor=2*opt.scale)
net = net.cuda()
...
for test_data in tqdm(test_dataloader):
    ...
    with torch.no_grad():
        ...
        # Save image
        if opt.save_image:
            utils.save_RGB(HR_RGB_cor, str(opt.scale), test_data['RGB_gt_name'], 'DBSR')
