...
parser.add_argument('--model', type=str, default='model_RawVSR', help='base model')
...
from models_RawVSR.model_RawVSR import RawVSR
import utils
...
# load model
net = RawVSR(nframes=opt.N_frames, scale=opt.scale)
net = net.cuda()
...
for test_data in tqdm(test_dataloader):
    ...
    with torch.no_grad():
        ...
        LR_RGB_gt = test_data['LR_RGB'].cuda()
        HR_RGB_gt = test_data['HR_RGB'].cuda()

        HR_RGB, lin_HR_RGB = net(LRs_RAW, LR_RGB_gt)
        HR_RGB_cor = color_correction(HR_RGB_gt, LR_RGB_gt, HR_RGB, opt.scale)
        ...
        # Save image
        if opt.save_image:
            utils.save_RGB(HR_RGB_cor, str(opt.scale), test_data['RGB_gt_name'], 'RawVSR')
    
for i in rgb_cor_ssim_list:
    print(i)
